//! Float-vector similarity search, ported from
...
/// Searches for every (approximate) float vector above a similarity threshold.
///
/// Equivalent to `org.apache.lucene.search.FloatVectorSimilarityQuery`.
#[derive(Debug, Clone)]
...
impl FloatVectorSimilarityQuery {
    ...
    ///
    /// Equivalent to
    /// `new FloatVectorSimilarityQuery(String, float[], float, float, Query, KnnSearchStrategy)`.
    ...
    /// when `result_similarity` or `decay` is `NaN`, when `decay` is outside
    /// `[0, 1]`, or when `target` holds a non-finite value.
    ...
    ///
    /// Equivalent to
    /// `new FloatVectorSimilarityQuery(String, float[], float, float, Query)`.
    ///
    /// # Errors
    ///
    ...
    ///
    /// Equivalent to
    /// `new FloatVectorSimilarityQuery(String, float[], float, Query)`.
    ///
    /// # Errors
    ///
    /// As [`with_strategy`](Self::with_strategy).
    ...
    ///
    /// Equivalent to `new FloatVectorSimilarityQuery(String, float[], float)`.
    ///
    /// # Errors
    ///
    /// As [`with_strategy`](Self::with_strategy).
    pub fn new(field: impl Into<String>, target: Vec<f32>, result_similarity: f32) -> Result<Self> {
...
impl AbstractVectorSimilarityQueryImpl for FloatVectorSimilarityQuery {
    ...
    fn create_vector_scorer(
        ...
        let Some(vector_values) = context
            .leaf_reader()
            .get_float_vector_values(self.base.get_field())?
        else {
            return Ok(None);
        };
    ...
    fn approximate_search(
        ...
        context.leaf_reader().search_nearest_vectors(
            self.base.get_field(),
            &self.target,
            collector.as_mut(),
            accept_docs,
...
impl Query for FloatVectorSimilarityQuery {
    fn to_query_string(&self, field: &str) -> String {
        format!(
            "FloatVectorSimilarityQuery[field={} target=[{:?}...] resultSimilarity={:?} decay={:?} filter={}]",
            field,
    ...
    fn query_eq(&self, other: &dyn Query) -> bool {
        match other.as_any().downcast_ref::<FloatVectorSimilarityQuery>() {
