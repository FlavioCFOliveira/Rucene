//! Byte-vector nearest-neighbour search, ported from
...
use crate::index::{check_byte_field, FieldInfo, LeafReaderContext};
...
/// Finds the `k` nearest documents to a target byte vector.
///
/// Equivalent to `org.apache.lucene.search.KnnByteVectorQuery`.
///
...
pub struct KnnByteVectorQuery {
    ...
    ///
    /// Equivalent to the `protected final byte[] target` field.
    target: Vec<u8>,
}
...
impl KnnByteVectorQuery {
    ...
    ///
    /// Equivalent to `new KnnByteVectorQuery(String, byte[], int)`.
    ///
    /// # Errors
    ///
    /// As [`with_strategy`](Self::with_strategy).
    pub fn new(field: impl Into<String>, target: Vec<u8>, k: i32) -> Result<Self> {
    ...
    /// accepts.
    ///
    /// Equivalent to `new KnnByteVectorQuery(String, byte[], int, Query)`,
    ...
    ///
    /// Equivalent to
    /// `new KnnByteVectorQuery(String, byte[], int, Query, KnnSearchStrategy)`.
    ...
    ///
    /// Equivalent to `KnnByteVectorQuery.getTargetCopy()`.
...
impl AbstractKnnVectorQueryImpl for KnnByteVectorQuery {
    ...
    fn approximate_search(
        ...
        let Some(byte_vector_values) = reader.get_byte_vector_values(self.base.get_field())? else {
            check_byte_field(reader.as_ref(), self.base.get_field())?;
            return Ok(empty_top_docs());
        };
        if knn_collector.k().min(byte_vector_values.size()) == 0 {
            return Ok(empty_top_docs());
        }
        reader.search_nearest_vectors_byte(
            self.base.get_field(),
            &self.target,
    ...
    fn create_vector_scorer(
        ...
        let reader = context.leaf_reader();
        let Some(vector_values) = reader.get_byte_vector_values(self.base.get_field())? else {
            check_byte_field(reader.as_ref(), self.base.get_field())?;
            return Ok(None);
        };
...
impl Query for KnnByteVectorQuery {
    fn to_query_string(&self, _field: &str) -> String {
        let mut buffer = String::from("KnnByteVectorQuery:");
    ...
    fn query_eq(&self, other: &dyn Query) -> bool {
        match other.as_any().downcast_ref::<KnnByteVectorQuery>() {
