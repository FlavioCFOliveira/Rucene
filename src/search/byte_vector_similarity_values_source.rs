//! Byte vector similarity as a value source, ported from
...
use crate::index::{check_byte_field, LeafReaderContext};
...
/// vector and a `KnnByteVectorField`.
///
/// Equivalent to the package-private
/// `org.apache.lucene.search.ByteVectorSimilarityValuesSource`.
#[derive(Debug, Clone)]
pub struct ByteVectorSimilarityValuesSource {
    query_vector: Vec<u8>,
    field_name: String,
}

impl ByteVectorSimilarityValuesSource {
    /// Creates a source over `field_name` for the given query vector.
    ///
    /// Equivalent to
    /// `new ByteVectorSimilarityValuesSource(byte[], String)`.
    pub fn new(vector: Vec<u8>, field_name: impl Into<String>) -> Self {
        Self {
            query_vector: vector,
...
impl SegmentCacheable for ByteVectorSimilarityValuesSource {
    fn is_cacheable(&self, _ctx: &LeafReaderContext) -> bool {
        true
    }
}

impl VectorSimilarityValuesSource for ByteVectorSimilarityValuesSource {
    ...
    fn get_scorer(&self, ctx: &LeafReaderContext) -> Result<Option<Box<dyn VectorScorer>>> {
        ...
        let vector_values = reader.get_byte_vector_values(&self.field_name)?;
        match vector_values {
            None => {
                check_byte_field(reader.as_ref(), &self.field_name)?;
                Ok(None)
            }
...
impl DoubleValuesSource for ByteVectorSimilarityValuesSource {
    fn get_values<'a>(
        self: Arc<Self>,
    ...
    fn source_eq(&self, other: &dyn DoubleValuesSource) -> bool {
        match other
            .as_any()
            .downcast_ref::<ByteVectorSimilarityValuesSource>()
        {
            Some(other) => {
    ...
    fn source_hash(&self) -> u64 {
        hash_of(&(&self.field_name, &self.query_vector))
    }
